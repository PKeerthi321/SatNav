...
def parse_rinex_nav(navfile):
    eph = {}
    try:
        with _open_text_file(navfile) as f:
            lines = f.readlines()
    except Exception:
        return eph
    idx = 0
    while idx < len(lines) and 'END OF HEADER' not in lines[idx]:
        idx += 1
    idx += 1
    while idx < len(lines) - 7:
        block = lines[idx:idx+8]
        header = block[0]
        prn = header[0:3].strip()
        if prn == '':
            tokens = header.split()
            if len(tokens) > 0:
                prn = tokens[0]
        ...
        for b in block:
            if len(b) > 3:
                parts = [b[i:i+19].replace('D', 'E') for i in range(3, len(b), 19)]
                for p in parts:
                    try:
        ...
        if len(nums) >= 22:
            try:
                e = {}
                e['M0'] = nums[3]; e['deltaN'] = nums[2]
                e['e']  = nums[4]; e['sqrtA']  = nums[5]
                e['omega0'] = nums[7]; e['w'] = nums[9]
                e['Crs'] = nums[12]; e['Crc'] = nums[10]
                e['Cis'] = nums[15]; e['Cic'] = nums[13]
                e['i0']  = nums[11]; e['OmegaDot'] = nums[14]
                e['Toe'] = nums[16]; e['A'] = e['sqrtA'] ** 2
                e['Cuc'] = nums[6];  e['Cus'] = nums[8]
                eph[prn] = e
            except Exception:
                pass
        idx += 8
...
def satpos_placeholder_from_id(sid, epoch_time, ref_ecef):
    ...
    az = (seed * 37) % 360
    el = 20 + (seed % 60)
    R  = 20200000.0 if sid.startswith('G') else 35786000.0
...
def satpos_from_eph(e, epoch_time):
    ...
    x_orb = r * math.cos(u)
    y_orb = r * math.sin(u)
    Omega0   = float(e.get('omega0', 0.0))
    OmegaDot = float(e.get('OmegaDot', 0.0))
...
def compute_sat_positions_from_nav(navgps_files, navirnss_files, epoch_time, sat_list):
    ...
    for f in (navgps_files or []):
        try: eph.update(parse_rinex_nav(f))
        except: pass
    ...
    for s in sat_list:
        Sat.append(s)
        Sys.append(s[0] if len(s) > 0 else '?')
